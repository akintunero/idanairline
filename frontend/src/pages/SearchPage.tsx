...
import { Clock, Plane, ChevronRight, ArrowUpDown, Wifi, Utensils, Luggage } from 'lucide-react';
...
export default function SearchPage({ searchParams, onSearch, onSelectFlight, isDark }: SearchPageProps) {
  ...
  const searchFlights = async (params: SearchParams) => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/flights/search?${paramsObj}`);
      const data = await res.json();
      if (data.success && data.data?.flights) {
        const mapped: Flight[] = data.data.flights.map((f: APIFlight) => {
          ...
          return {
            ...
            origin: getAirport(f.origin) || { code: f.origin, city: f.origin, name: f.origin, country: '', region: '' },
            destination: getAirport(f.destination) || { code: f.destination, city: f.destination, name: f.destination, country: '', region: '' },
    ...
    <div className={`${bg} min-h-screen`}>
      <div className={`${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'} border-b`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <FlightSearchForm initialParams={searchParams || undefined} onSearch={onSearch} isDark={isDark} compact />
        </div>
      </div>
        ...
        {searchParams && (
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-1">
              <h1 className={`text-2xl font-bold ${textPrimary}`}>{originAirport?.city || searchParams.origin}</h1>
              <Plane className={`w-5 h-5 ${isDark ? 'text-sky-400' : 'text-sky-500'}`} />
              <h1 className={`text-2xl font-bold ${textPrimary}`}>{destAirport?.city || searchParams.destination}</h1>
            </div>
            <p className={`text-sm ${textSecondary}`}>{searchParams.departureDate} &bull; {searchParams.passengers} passenger{searchParams.passengers > 1 ? 's' : ''} &bull; {searchParams.seatClass.charAt(0).toUpperCase() + searchParams.seatClass.slice(1)}</p>
          </div>
        )}
        <div className={`flex flex-wrap items-center gap-3 mb-5 p-3 rounded-xl border ${cardBg}`}>
          ...
          {(['price', 'duration', 'departure'] as const).map(opt => (
            <button key={opt} onClick={() => setSortBy(opt)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${sortBy === opt ? 'bg-sky-600 text-white' : (isDark ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}`}>
              {opt.charAt(0).toUpperCase() + opt.slice(1)}
            </button>
          ))}
          <div className={`w-px h-4 mx-1 ${isDark ? 'bg-gray-700' : 'bg-gray-300'}`} />
          {(['all', 'economy', 'business', 'first'] as const).map(opt => (
            <button key={opt} onClick={() => setFilterClass(opt)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filterClass === opt ? 'bg-sky-600 text-white' : (isDark ? 'bg-gray-700 text-gray-300 hover:bg-gray-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}`}>
        ...
        {!loading && (
          <div className="space-y-3">
            {sortedFlights.map((flight, idx) => (
              <FlightCard key={flight.id} flight={flight} onSelect={() => searchParams && onSelectFlight(flight, searchParams)} isDark={isDark} isCheapest={idx === 0} />
            ))}
...
function FlightCard({ flight, onSelect, isDark, isCheapest }: { flight: Flight; onSelect: () => void; isDark: boolean; isCheapest: boolean }) {
  const cardBg = isDark ? 'bg-gray-800 border-gray-700 hover:border-sky-500/50' : 'bg-white border-gray-200 hover:border-sky-400/60';
  const textPrimary = isDark ? 'text-white' : 'text-gray-900';
  const textSecondary = isDark ? 'text-gray-400' : 'text-gray-500';
  const classColors: Record<string, string> = { economy: 'bg-emerald-100 text-emerald-700', business: 'bg-amber-100 text-amber-700', first: 'bg-sky-100 text-sky-700' };
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-1">
          ...
          <div className="flex-1 flex flex-col items-center gap-0.5 px-2">
            <div className={`text-xs ${textSecondary} flex items-center gap-1`}><Clock className="w-3 h-3" />{flight.duration}</div>
        ...
        <div className="hidden lg:flex items-center gap-3">
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${classColors[flight.seatClass]}`}>{flight.seatClass.charAt(0).toUpperCase() + flight.seatClass.slice(1)}</span>
          <div className="flex gap-1.5">
            <Wifi className={`w-3.5 h-3.5 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
            <Utensils className={`w-3.5 h-3.5 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
            <Luggage className={`w-3.5 h-3.5 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
          </div>
        ...
        <div className="flex items-center justify-between lg:flex-col lg:items-end gap-2 lg:w-36">
          <div>
            {isCheapest && <div className="text-xs font-semibold text-emerald-600 mb-0.5">Best Price</div>}
            <div className={`text-2xl font-bold ${textPrimary}`}>${flight.price.toLocaleString()}</div>
            <div className={`text-xs ${textSecondary}`}>per person</div>
          </div>
          <button onClick={onSelect} className="flex items-center gap-1.5 px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-sm font-semibold transition-colors group-hover:shadow-md">
